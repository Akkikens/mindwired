...
**PRIMARY — What They Found Inside the Deepest Animals on Earth** (51 chars)

| Title | Chars | curiosity/first-3/honesty/mobile | Avg |
|---|---|---|---|
| What They Found Inside the Deepest Animals on Earth | 51 | 9 / 9 / 10 / 7 | **8.75** — ship |
| What They Found Seven Miles Down | 32 | 8 / 9 / 9 / 10 | **9.0** — variant A |
| 100% of the Animals at Earth's Deepest Point Had Plastic | 55 | 8 / 9 / 10 / 6 | **8.25** — variant B |

The 9.0 is shorter and scores higher on mobile length, but it is generic — it could be
about anything seven miles down. The shipped title front-loads the exact phrase winning
in this lane ("What They Found…") and "Inside" carries the body-horror turn that is the
actual story. It also matches the rendered title card. Variant A goes into the title
test after the thumbnail test settles.

**Suggested-adjacency** (what this should sit next to): *Mariana Trench: What They Found
In The Deepest Place On Earth* (4.0M / 29.2K subs, Mar 2026) and *What NASA Actually
Found At The Bottom Of The Ocean* (1.24M / 30.5K, Mar 2026).
...
| File | Asset | Why |
|---|---|---|
| `out/thumbs/hadalpollution_A.png` | *Hirondellea gigas*, the real Mariana amphipod (Commons, CC BY-SA 2.5) | pale luminous organism on pure black — the strongest 170px read of any thumbnail built for this channel |
| `out/thumbs/hadalpollution_B.png` | *Pseudoliparis swirei*, the real hadal snailfish (Commons, CC BY 3.0) | the actual deepest vertebrate, eye visible, figure artifacts cropped out |
| `out/thumbs/hadalpollution_C.png` | a frame from NOAA's "Crab Defending Bait" lander clip | a real scavenger on a bait platform under ROV light |

A fourth was built and killed: the Mariana bathymetry map. At 170px it is a green smear
with an axis colorbar — it reads as a figure, not an image.

**Test & Compare:** all three at publish. Winner on **watch-time share, not CTR**.
...
mindwired has already made a Mariana Trench video and it did **197 views**. Demand was
never the problem — the winners in the same sweep are the same subject. That episode
promised a mood ("scarier than space"); this one promises a payload. If this also
underperforms, the conclusion is NOT "deep ocean doesn't work" — it is that the channel
cannot yet win a cold start on ANY topic, and the next lever is the Shorts funnel
(still zero published), not another topic swap.

Second risk: at ~12:48 this is shorter than the channel's recent episodes. The evidence
base is tight and padding it would have reintroduced the repetition problem, but if
retention is strong and watch-time is the limiter, the fix is a longer script on the
