#!/usr/bin/env python3
"""
Offline placeholder manifest for "The Scariest Places In The Universe" so the
episode can be built + previewed WITHOUT the ElevenLabs key. Estimates per-clip
duration + word timings from the narration text and writes silent mp3s. The real
scripts/scariest/build_audio.py overwrites these (manifest 'estimated' flag triggers
regen) with Brian-voiced audio + forced-aligned timings; the video re-times itself.

Run: python3 scripts/scariest/estimate_manifest.py
"""
import json
import re
import subprocess
...
def main():
    ...
    for clip in B.NARRATION:
        ...
        print(f"  {clip['id']:<16} {c['dur']:6.1f}s  {'(real)' if not c.get('estimated') else ''}")
