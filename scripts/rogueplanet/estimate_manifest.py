#!/usr/bin/env python3
"""
Offline placeholder manifest for the rogue-planet episode so the comp can be built
and previewed WITHOUT the ElevenLabs key. Estimates per-clip duration + word
timings from the narration text and writes silent mp3s. The real
scripts/rogueplanet/build_audio.py overwrites these (the 'estimated' flag triggers
regen) with George-voiced audio + word timings; the video re-times itself.

Run: python3 scripts/rogueplanet/estimate_manifest.py
"""
import json
import re
import subprocess
...
import build_audio as B  # noqa: E402  (data only; eleven key is lazy)

REPO = Path(__file__).resolve().parent.parent.parent
OUT = REPO / "public" / B.SLUG / "audio"
OUT.mkdir(parents=True, exist_ok=True)

WPS = 2.4           # George's effective words/sec incl. dramatic delivery
...
def main():
    ...
    for clip in B.NARRATION:
        ...
        print(f"  {clip['id']:<12} {c['dur']:6.1f}s  {'(real)' if not c.get('estimated') else ''}")
